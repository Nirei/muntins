 ...
 */
export function resolveStyle(partial) {
 ...
 */
function collectLayoutChildren(children, parent) {
...
function buildLayoutTree(node, parent) {
    const box = {
        node,
...
function buildTopDownQueue(root) {
    const queue = [root];
    const result = [];
    let head = 0;
...
function clamp(value, minBound, maxBound) {
    const clamped = Math.max(minBound, value);
    return maxBound !== null ? Math.min(maxBound, clamped) : clamped;
}
function clampBoxSize(box) {
    const minW = typeof box.style.minWidth === "number"
        ? box.style.minWidth
        : box.autoMinWidth;
    const minH = typeof box.style.minHeight === "number"
        ? box.style.minHeight
        : box.autoMinHeight;
    const maxW = box.style.maxWidth !== null && box.style.maxWidth < minW
        ? null
        : box.style.maxWidth;
    const maxH = box.style.maxHeight !== null && box.style.maxHeight < minH
        ? null
        : box.style.maxHeight;
    box.width = clamp(box.width, minW, maxW);
    box.height = clamp(box.height, minH, maxH);
}
function isSpaceJustify(justify) {
    return (justify === "space-between" ||
        justify === "space-around" ||
        justify === "space-evenly");
}
function getMainPadding(style, isRow) {
    return isRow
        ? style.paddingStart + style.paddingEnd
        : style.paddingTop + style.paddingBottom;
}
function getCrossPadding(style, isRow) {
    return isRow
        ? style.paddingTop + style.paddingBottom
        : style.paddingStart + style.paddingEnd;
}
function getMainMargin(style, isRow) {
    return isRow
        ? style.marginStart + style.marginEnd
        : style.marginTop + style.marginBottom;
}
function getCrossMargin(style, isRow) {
    return isRow
        ? style.marginTop + style.marginBottom
        : style.marginStart + style.marginEnd;
}
function borderSize(enabled) {
    return enabled ? 1 : 0;
}
function getMainBorder(style, isRow) {
    return isRow
        ? borderSize(style.borderStart) + borderSize(style.borderEnd)
        : borderSize(style.borderTop) + borderSize(style.borderBottom);
}
function getCrossBorder(style, isRow) {
    return isRow
        ? borderSize(style.borderTop) + borderSize(style.borderBottom)
        : borderSize(style.borderStart) + borderSize(style.borderEnd);
}
function getWidthPaddingBorder(style) {
    return (style.paddingStart +
        style.paddingEnd +
        borderSize(style.borderStart) +
        borderSize(style.borderEnd));
}
function getHeightPaddingBorder(style) {
...
function createLine(items, gap, isRow) {
    let mainSize = 0;
    let crossSize = 0;
...
function collectLines(box, visibleChildren, isRow) {
    const style = box.style;
    const availableMain = (isRow ? box.width : box.height) -
    ...
    for (const child of visibleChildren) {
        ...
        const wouldOverflow = currentMainSize + gapBefore + childTotalMain > availableMain;
...
function calculateIntrinsicSize(box, axis) {
    const style = box.style;
    const isRow = style.flexDirection === "row";
    const isMainAxis = (axis === "width" && isRow) || (axis === "height" && !isRow);
    const paddingBorder = axis === "width"
    ...
    if (isMainAxis) {
        ...
        for (let i = 0; i < visibleChildren.length; i++) {
            ...
            if (i > 0)
                contentSize += style.gap;
        }
    }
    else {
        // Cross axis: max of children sizes
        for (const child of visibleChildren) {
            contentSize = Math.max(contentSize, child[axis] + getCrossMargin(child.style, isRow));
        }
    }
 ...
 */
function resolveIntrinsicSizeRecursive(box, availableWidth) {
    const style = box.style;
    ...
    const contentWidth = Math.max(0, box.width -
        style.paddingStart -
        style.paddingEnd -
        borderSize(style.borderStart) -
        borderSize(style.borderEnd));
    ...
    const widthSetByFlexBasis = typeof style.flexBasis === "number" && parentIsRow;
    const heightSetByFlexBasis = typeof style.flexBasis === "number" && parentIsColumn;
...
function distributeGrowForLine(items, availableSpace, isRow) {
    if (availableSpace <= 0)
        return;
    const flexChildren = [];
    const weights = [];
...
function distributeShrinkForLine(items, overflow, isRow) {
    if (overflow <= 0)
        return;
    const shrinkChildren = [];
    const weights = [];
    const effectiveMins = [];
    for (const child of items) {
        if (child.style.flexShrink > 0) {
            ...
            const effectiveMin = typeof explicitMin === "number" ? explicitMin : autoMin;
...
function calculateAvailableMainSpaceForLine(box, line) {
    const style = box.style;
    const isRow = style.flexDirection === "row";
    const availableMain = (isRow ? box.width : box.height) -
        getMainPadding(style, isRow) -
...
function applyJustifyContentForLine(box, line, isRow) {
    const style = box.style;
...
function applyAlignItemsForLine(box, line, crossStart, lineCrossSize, isRow) {
    const style = box.style;
...
function positionAbsoluteChildren(box, absoluteChildren) {
    const style = box.style;
    ...
    for (const child of absoluteChildren) {
        ...
        if (childStyle.width === "auto" &&
            typeof childStyle.start === "number" &&
            typeof childStyle.end === "number") {
            const newWidth = Math.max(0, contentWidth - childStyle.start - childStyle.end);
        ...
        if (childStyle.height === "auto" &&
            typeof childStyle.top === "number" &&
            typeof childStyle.bottom === "number") {
            const newHeight = Math.max(0, contentHeight - childStyle.top - childStyle.bottom);
        ...
        if (sizeChanged) {
            ...
            const absoluteContentWidth = Math.max(0, child.width -
                childBorderStart -
                childBorderEnd -
...
function resolveFlexAndPosition(box) {
    ...
    const containerCrossSize = (isRow ? box.height : box.width) - crossPadding - crossBorder;
...
function finalizePositions(box, parentScreenX, parentScreenY) {
 ...
 */
export function computeLayout(node, availableWidth, availableHeight) {
    // 1. Build internal tree with resolved styles
