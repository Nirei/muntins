import { createSignal, untrack, } from "../signals.js";
/**
...
export class FocusManager {
     ...
     */
    registerSubtreeFocusables(subtreeRoot) {
     ...
     */
    unregisterSubtreeFocusables(subtreeRoot) {
        const scope = this.findParentScope(subtreeRoot);
        const nodesToRemove = [];
     ...
     */
    restoreFocusAfterDisposal(subtreeRoot) {
        let current = subtreeRoot._parent;
        while (current) {
     ...
     */
    static collectFocusableInScope(node, scope) {
        const nodeScope = node._focusScope;
    ...
    findParentScope(node) {
        let current = node._parent;
        while (current) {
            const nodeScope = current._focusScope;
            if (nodeScope) {
                return nodeScope;
            }
            current = current._parent;
    ...
    focusNavigate(scope, direction) {
        ...
        const atBoundary = direction === 1 ? targetIndex >= focusableNodes.length : targetIndex < 0;
        if (atBoundary) {
    ...
    findScopeContaining(rootScope, node) {
    ...
    static countFocusablesInAncestors(scope) {
        let count = 0;
        let current = scope;
        while (current) {
    ...
    findScopeForNode(target, defaultScope) {
        let current = target;
        while (current) {
            const scope = current._focusScope;
