 ...
 */
export function displayWidth(char) {
    if (char === "")
        return 0;
    ...
    if (inRanges(ZERO_WIDTH_RANGES, code))
        return 0;
    // Binary search for wide
    if (inRanges(WIDE_RANGES, code))
        return 2;
    return 1;
}
 ...
 */
export const DEFAULT_COLOR = { type: "default" };
...
export class Buffer {
    ...
    back; // Render target for this frame
    _width;
    _height;
    // Style state (persists across flush calls)
    _styleFg = COLOR_DEFAULT;
    _styleBg = COLOR_DEFAULT;
    _styleModifiers = 0;
     ...
     */
    set(x, y, symbol, fg, bg, modifiers) {
    ...
    setInternal(x, y, symbol, fg, bg, modifiers) {
     ...
     */
    writeText(x, y, text, fg, bg, modifiers) {
     ...
     */
    fillRect(x, y, width, height, symbol, fg, bg, modifiers) {
    ...
    emitStyleDiff(parts, cell) {
        ...
        if (addedModifiers & BOLD)
            codes.push(1);
        if (addedModifiers & DIM)
            codes.push(2);
        if (addedModifiers & ITALIC)
            codes.push(3);
        if (addedModifiers & UNDERLINE)
            codes.push(4);
        if (addedModifiers & BLINK)
            codes.push(5);
        if (addedModifiers & INVERSE)
            codes.push(7);
        if (addedModifiers & HIDDEN)
            codes.push(8);
        if (addedModifiers & STRIKETHROUGH)
            codes.push(9);
    ...
    appendColorCodes(codes, packed, isForeground) {
        const type = packed & COLOR_TYPE_MASK;
        const value = packed & 0x00_ffffff;
