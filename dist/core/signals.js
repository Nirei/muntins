 ...
 */
function unlinkSourceFromObserver(source, observer) {
    if (source.observers) {
...
function notifyObservers(node) {
    ...
    const effectsToSchedule = new Set();
    for (const observer of node.observers) {
 ...
 */
function collectStaleEffects(node, effects) {
    if (!node.observers)
        return;
    for (const observer of node.observers) {
 ...
 */
function executeWithTracking(node) {
    if (!node.fn)
        return;
 ...
 */
function runTopLevelEffect(node) {
    effectDepth++;
    let iterations = 0;
    try {
        pendingEffects.add(node);
        ...
        if (iterations >= MAX_ITERATIONS) {
            throw new Error("Effect exceeded maximum iterations - possible infinite loop");
        }
    }
    finally {
        effectDepth--;
...
export function createMemo(fn) {
    const node = {
        fn,
        value: undefined, // Will be computed on first read
 ...
 */
export function createRoot(fn, options) {
