...
import { DIM, INVERSE, graphemeDisplayWidth, graphemes, } from "../core/buffer.js";
import { Box } from "../core/components/Box.js";
import { ScrollArea } from "../core/components/ScrollArea.js";
import { isPrintable } from "../core/input.js";
import { DEFAULT_FLEX_STYLE, } from "../core/layout.js";
import { rectContains } from "../core/rects.js";
...
function cursorToVisualPos(text, cursorPos, width) {
    ...
    for (let row = 0; row < vlines.length; row++) {
        ...
        if (cursorPos === endPos) {
            ...
            if (next && next.globalGraphemeStart === cursorPos)
                continue;
            return { row, col: cursorPos - vl.globalGraphemeStart };
        }
    }
...
function visualPosToCursor(text, row, col, width) {
    const vlines = getVisualLines(text, width);
 ...
 */
function posToCharIndex(text, graphemePos) {
    let charIndex = 0;
    let graphemeIndex = 0;
    for (const grapheme of graphemes(text)) {
        if (graphemeIndex >= graphemePos)
            break;
...
export function Textarea(props) {
    ...
    const getWidth = () => resolve(props.width) ?? theme("textarea").width;
    ...
    const focusedNodeAccessor = ctx?.app.focusedNode ?? null;
    let focusableNode;
    let lastScreenX = 0;
    let lastScreenY = 0;
    ...
    const contentWidth = () => {
        ...
        if (getMaxHeight() === undefined)
            return getWidth() - ps - pe;
    ...
    createEffect(() => {
        ...
        if (maxHeight === undefined)
            return; // No scrolling without maxHeight
        const val = getValue();
    ...
    const handleKeyPress = (key) => {
        ...
        if (key.name === "down") {
            ...
            if (visualPos.row < vlines.length - 1) {
                ...
                const effectiveCol = onOverflow ? cw : visualPos.col;
                let targetRow = visualPos.row + 1;
                if (onOverflow && targetRow < vlines.length - 1)
                    targetRow++;
                setCursorPos(visualPosToCursor(val, targetRow, effectiveCol, cw));
            }
            return true;
        ...
        if (key.name === "end" || (key.ctrl && key.name === "e")) {
            if (isMultiline) {
                ...
                setCursorPos(visualPosToCursor(val, visualPos.row, vl.graphemeCount, cw));
            }
            else {
                setCursorPos(len);
            }
    ...
    const contentNode = new Node({
        ...
        measure(_availableWidth, _availableHeight) {
            const val = getValue();
            const cw = contentWidth();
            const vlines = isMultiline
                ? getVisualLines(val, cw)
        ...
        render(bounds, buffer, inherited, clip) {
            ...
            const cursorInverse = theme("textarea--cursor").inverse;
            const placeholderDim = theme("textarea--placeholder").dim;
            const disabledDim = theme("textarea--disabled").dim;
            const hScroll = isMultiline ? 0 : scrollLeft();
            const cw = contentWidth();
            if (val.length === 0 && placeholder.length > 0) {
                renderTextareaContent(buffer, x, y, width, height, placeholder, fg, bg, placeholderDim, false, -1, hScroll, clip, cursorInverse, isMultiline ? cw : 0);
                return;
            }
            const showCursor = !disabled && isFocused();
            renderTextareaContent(buffer, x, y, width, height, val, fg, bg, disabled && disabledDim, showCursor, pos, hScroll, clip, cursorInverse, isMultiline ? cw : 0);
        },
    });
    const maxHeight = getMaxHeight();
    if (maxHeight !== undefined) {
 ...
 */
function renderTextareaContent(buffer, x, y, width, height, text, fg, bg, dim, showCursor, cursorPos, scrollLeft = 0, clip, cursorInverse = true, cw = 0) {
    const vlines = getVisualLines(text, cw);
    const inClip = (cx, cy) => !clip || rectContains(clip, cx, cy);
    ...
    for (let row = 0; row < Math.min(vlines.length, height); row++) {
        ...
        for (const grapheme of graphemes(vl.text)) {
            ...
            const isVisible = visibleStart + graphemeWidth > 0 && visibleStart < width;
            if (isVisible) {
            ...
            graphemeIdx++;
            if (displayCol - scrollLeft >= width)
                break;
        }
        if (showCursor &&
            row === cursorVisualRow &&
            cursorVisualCol === vl.graphemeCount) {
            const cursorDisplayCol = displayWidthToPosition(vl.text, cursorVisualCol) - scrollLeft;
            if (cursorDisplayCol >= 0 &&
                cursorDisplayCol < width &&
                inClip(x + cursorDisplayCol, screenY)) {
                buffer.set(x + cursorDisplayCol, screenY, " ", fg, bg, cursorInverse ? INVERSE : 0);
            }
        }
    }
}
