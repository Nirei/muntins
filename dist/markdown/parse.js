...
export function parse(markdown) {
    ...
    return tokens.flatMap((token) => blockFromToken(token));
}
    ...
    switch (token.type) {
        ...
        case "blockquote":
            return [
                {
                    type: "blockquote",
                    children: (token.tokens ?? []).flatMap((child) => blockFromToken(child)),
                },
            ];
        case "hr":
        ...
        case "list": {
            const items = (token.items ?? []).map(listItemFromToken);
            return [
                {
                    type: "list",
...
function listItemFromToken(token) {
    let task;
    if (token.task === true) {
    ...
    const children = rawTokens.flatMap(blockFromToken);
...
function stripTaskPrefix(children) {
...
function tableFromToken(token) {
    ...
    const cells = rows.map((row) => row.map((cell) => inlineChildren(cell.tokens)));
...
function inlineChildren(tokens) {
    ...
    if (!Array.isArray(tokens))
        return [];
    return tokens.flatMap(inlineFromToken);
}
function inlineFromToken(token) {
    switch (token.type) {
        case "text":
        case "escape": {
...
function textFromTokens(tokens) {
    ...
    if (!Array.isArray(tokens))
        return null;
    const flat = (list) => list
        .map((token) => Array.isArray(token.tokens)
        ? flat(token.tokens)
        : String(token.text ?? ""))
        .join("");
    const text = flat(tokens);
